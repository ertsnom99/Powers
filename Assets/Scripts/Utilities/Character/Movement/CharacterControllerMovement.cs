...
public class CharacterControllerMovement : MonoSubscribable<IStaminaSubscriber>
{
    ...
    private void RotateCharacterWithInput(Inputs inputs)
    {
        Quaternion previousRotation = transform.rotation;
	...
	private void CalculateLocalXVelocity()
    {
        ...
        float currentMaxVelocityX = m_maxXWalkSpeed * m_currentMovementInput.x * m_currentModifiers["sideStepModifier"] * m_currentModifiers["runningModifier"] * m_globalModifier;

        // If the character moves without having achived is maximum horizontal velocity
        if (m_currentMovementInput.x != 0 && previousVelocityX != currentMaxVelocityX && (!IsAirborne || m_controlWhileAirborne))
        {
            m_localVelocity.x += Mathf.Abs(m_currentMovementInput.x) * m_acceleration * Mathf.Sign(currentMaxVelocityX - m_localVelocity.x) * m_currentModifiers["sideStepModifier"] * m_currentModifiers["airborneModifier"] * m_currentModifiers["runningModifier"] * Time.deltaTime;
        ...
        else if (m_currentMovementInput.x == 0 && m_characterController.isGrounded && previousVelocityX != 0)
        {
    ...
    private void CalculateLocalZVelocity()
    {
        ...
        float currentMaxVelocityZ = m_maxZWalkSpeed * m_currentMovementInput.y * m_currentModifiers["backwardModifier"] * m_currentModifiers["runningModifier"] * m_globalModifier;

        // If the character moves without having achived is maximum vertical velocity
        if (m_currentMovementInput.y != 0 && previousVelocityZ != currentMaxVelocityZ && (!IsAirborne || m_controlWhileAirborne))
        {
            m_localVelocity.z += Mathf.Abs(m_currentMovementInput.y) * m_acceleration * Mathf.Sign(currentMaxVelocityZ - m_localVelocity.z) * m_currentModifiers["backwardModifier"] * m_currentModifiers["airborneModifier"] * m_currentModifiers["runningModifier"] * Time.deltaTime;
        ...
        else if (m_currentMovementInput.y == 0 && m_characterController.isGrounded && previousVelocityZ != 0)
        {
